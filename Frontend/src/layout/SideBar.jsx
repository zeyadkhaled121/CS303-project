...
const SideBar = ({
  ...
  let menuItems = [
    ...
    { name: "Proposals", icon: <FaLightbulb />, component: "Proposals" },
  ];
  ...
  return (
    <aside
      className={`fixed left-0 top-16 h-[calc(100vh-64px)] bg-[#358a74] text-white flex flex-col z-40 transition-all duration-500 shadow-xl ${isSideBarOpen ? "w-64" : "w-20"}`}
    >
      <button
        onClick={() => setIsSideBarOpen(!isSideBarOpen)}
        className="absolute -right-5 top-1/2 -translate-y-1/2 bg-[#358a74] text-white py-6 px-1 rounded-md shadow-lg opacity-0 hover:opacity-100 transition-opacity"
      >
      ...
      {isAuthenticated && (
        <div className="p-4 border-t border-white/10">
          <button
            ...
            className={`w-full py-3 px-4 rounded-xl flex items-center hover:bg-[#2c7360] transition-all ${isSideBarOpen ? "justify-start gap-4" : "justify-center"}`}
          >
